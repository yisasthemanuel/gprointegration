		...
		while (currentPage <= pageCount) {
			...
			for(AvailDriversResponseDriversInner driver: response.getDrivers()) {
				...
				if (hasBuenosAires && hasLagunaSeca) {
					log.info("BINGO!!! The driver with id {} has Buenos Aires and Laguna Seca as FT!!!", driverID);
				}
				else if (hasBuenosAires) {
					log.info("LINE!! The driver with id {} has Buenos Aires as FT!!!", driverID);
				}
				else if (hasLagunaSeca) {
					log.info("LINE!! The driver with id {} has Laguna Seca as FT!!!", driverID);
				}

				if (hasBuenosAires || hasLagunaSeca) {
					log.info("This is the driver name {} and experience {}", driverProfile.getDriName(), driverProfile.getExperience());
					log.info("==============");
					log.info("");
				}
			}

			log.info("Response pageCount({}), pageSize({}), pageIndex({})", response.getPageCount(), response.getPageSize(), response.getPageIndex());
			currentPage++;
			response = api.getAvailDrivers(lang, currentPage, minOA, maxOA, sort, sort2, sort3, agr, cha, con,
				mot, rep, sta, tal, tei, exp, age, minsal, offRange, wei).block();
		}
		log.info("Número de drivers consultados {}", driversRead);
	}
